...
- レイヤ名は 外形／機器／機器-内側／機器-投影／機器-型式／ダクト／DINレール／加工-丸穴／加工-タップ／加工-切り欠き／図面-注記／図面-風船／図面-部品表。
  **機器-内側**は面の内側に付けた機器（外から見えない）、**機器-投影**はほかの面に付けた機器の投影。
  どちらも SVG では破線（`stroke-dasharray`）になっているので、entities から自前で描くときも破線にする。
  **図面-風船**は機器に付けた丸囲みの番号（円・引き出し線・数字）、**図面-部品表**は図の右の余白に置いた
  No. / 型式 / 品名 / メーカー / 数量 の表（線と文字）。どちらも **cabinet_full（盤の外観）だけ**に入り、
  `extent` は表まで含んだ大きさになっている（縮尺はその extent で決めればよい）。中板のシートには入らない。
...
7. 色: layers[名前].color（#rrggbb）を線の色に使う。印刷が白黒のときは全部黒にする設定を用意する。
   ...
   cabinet_full（盤の外観）には「図面-風船」（機器の番号。同じ型式は 1 つの風船から各機器へ線）と
   「図面-部品表」（右の余白の表）が入っている。
   これは図の一部としてそのまま描けばよい（表の文字は日本語を含む）。
