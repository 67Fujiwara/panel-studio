...
| | 左ペイン | 右ペイン |
| --- | --- | --- |
| 中板 | ダクトレイアウト・段ごとの余白・クリアランス・使用部品 | 加工・チェック |
...
- **段は機器の行**で、横ダクトはその上下に付きます。だから **N 段なら横ダクトは N＋1 本**になり、
  数が1つずれて見えます。見出しに両方の本数を出し、各行に
  「1 段目（ダクト 1↔2 本目）」と、どのダクトを動かすのかを添えています
...
設計完了と同時に、図面を **DXF で4ファイル**出します（完了案件の行の「DXF」からも同じものが出ます）。
...
- レイヤは 外形／機器／機器-型式／ダクト／DINレール／**加工-丸穴**／**加工-タップ**／**加工-切り欠き**／図面-注記。
  加工だけ拾いたいときにレイヤで選べます
- ブラウザは連続ダウンロードを1回だけ確認するので、最初に「複数ファイルのダウンロードを許可」を選んでください
...
- **DXF 書き出し 4ファイル**（キャビネット／中板 × 機器つき／加工穴のみ・R12・Shift-JIS・レイヤ分け）
