...
**左に制御盤の展開図、右に BOM と CSV** の2列です。面を押すとその面のレイアウト画面に移ります。
展開図は実際の寸法比で描いているので、どの面がどれだけの大きさか一目で分かります。

対象は**7面**（上面／左側面／正面（扉）／右側面／**背面**／底面／中板）。
中板は箱の一部ではないので、展開図の下に離して破線で置いています。
...
目盛りは **10mm / 50mm / 100mm** の3段階で、100mm ごとに数値が出ます。
...
- 段ごとに**ダクトとの余白**を個別に決められます。指定しない段は共通の設定を使います
...
- 加工（穴あけ・切り欠き）
  ...
  - **丸穴（径指定）とタップ穴（M3/M4/M5/M6）を切り替え可能。** タップは下穴径を自動で入れるので
    径の入力は出ません。加工リスト CSV には「ねじ」列に呼びが出ます
  - 図では**タップ穴を二重丸**（外側が呼び径、内側が下穴）で描き、丸穴と見分けられます
  - 穴・タップ・切り欠きはすべて**実線**。切り欠きも**中心座標**で置きます
