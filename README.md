...
**配置した中身（ダクト・DINレール・機器）も、その面のセルの中に実寸で描きます。**
機器は分類の色で塗り分けます。「機器 12」という数だけでは、どの面がもう埋まっていて
どこが空いているか分からず、面を1つずつ開いて確かめる羽目になるためです。
この縮尺では文字が潰れるので、形と量だけを伝えます。
...
### AI 自動配置（準備済み・接続先は各自で設定）

使用部品を**どの段にどの順で並べるか**を Claude に決めさせる仕組みを入れてあります。
接続先を設定すると、中板の画面に「AI 自動配置」が出ます。

#### 何を AI に任せて、何を任せないか

**AI が決めるのは段割りと並び順だけで、座標は決めさせません。**

座標まで AI に出させると、クリアランス違反や重なりが混じったまま図になり、
人が全部検算する羽目になります。一方で段割りは
「主幹は上段・端子台は最下段・発熱物は上」といった**人の作法**の話で、
規則に書き下しにくい ＝ AI が役に立つところです。

出てきた段割りは既存の詰め込みに渡すので、座標・クリアランス・重なりの検査は
これまでどおり機械が持ちます。**AI の答えがそのまま図の不備になることはありません。**

```
盤サイズ＋使用部品  ──→  Claude  ──→  段割り(JSON)
                                          ↓
                              既存の詰め込み（座標を計算）
                                          ↓
                              クリアランス・重なりの検査
...
- **AI 自動配置**（段割りと並び順だけを Claude に決めさせ、座標と検査は機械が持つ）
