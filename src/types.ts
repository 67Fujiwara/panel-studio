...
export type DeviceSpec = {
  ...
  shape?: DeviceShape;
  /**
   * 側面から見た外形線（干渉確認用）。登録すると左右側面の図に薄く投影される。
   * 向きの約束: **中板側を左・扉側を右**に描いた図を取り込む（左側面ビューの向き）。
   * 外接寸法は 奥行き(d)×高さ(h) に拡縮されるので、絵の縮尺は問わない。
   */
  sideShape?: DeviceShape;
  /**
  ...
  /**
   * 止め金具（エンドストッパ）。隣の機器との離隔（メーカー指定・機器⇔機器）を**無視して密着**させる。
   * 未指定なら名前・型式に「ストッパ」「止め金具」「CLIPFIX」があれば止め金具とみなす。
   */
  stopper?: boolean;
  /** 発熱量。盤内温度上昇の計算に使う */
  heatW?: number;
...
/**
 * 型式欄のカンマ区切りを分解する。
 * 端子台に載せるリレーのように、**置くのは1個でも買い物は複数型式**の部品は、
 * 型式欄に「ソケット,リレー」とカンマで並べて書く。BOM はぜんぶの型式を
 * 1個ずつ数える（見積依頼CSV・単価引きにもそれぞれ独立した行で載る）。
 * 全角カンマも受ける。区切りが無ければそのまま1件。
 */
export function splitModels(model: string): string[] {
  const parts = model
    .split(/[,，]/)
    .map((s) => s.trim())
    .filter((s) => s !== '');
